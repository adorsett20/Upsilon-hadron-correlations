...
void TuneTestEnergy() {
  TFile *file = TFile::Open("PPCollisionUps1sWithTuneEnergyMid.root");
  TFile *file2 = TFile::Open("PPCollisionTuneEnergyMid.root");
  ...
  PPTupleUps->Draw("(ET-mu1pT-mu2pT)>>NCharUps", "NChar>2 && abs(deta1) < 1.93 && abs(deta2) < 1.93  && mu1pT > 4 && mu2pT > 4");
//  NCharUps->Scale(3.35e-6);
  ...
  PPTupleND->Draw("ET>>Tracks", "NChar > 0");
...
//	percent = .40;
  ...
  for(c = 0; NCharUps->GetBinCenter(30000-i) > TrackCent[c+1] && NCharUps->GetBinCenter(30000-i) <= TrackCent[c]; c++) {
        sum = 0;
        bins = 0;
        for(i = min; (NCharUps->GetBinCenter(30000-i) > TrackCent[c+1] && NCharUps->GetBinCenter(30000-i) <= TrackCent[c]) && i != 100000; i++) {
...
TrackAverage[2] = TrackAverage[2]/Tracks->GetMean();
 ...
 for(c = 3; c < 14; c++) {
  ...
  TestY = (10./NCharUps->Integral(0,25000))*NCharUps->Integral(38, 100);
  cout << "\n" << "\n" << TestY << "\n" << "\n";
